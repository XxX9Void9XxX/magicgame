...
function spawnCrates() {
  ...
  const abilities = ["ice","lightning","dark","light","poison","healing","water","lava","wind"];
  crates.push({ ...randomPos(), ability: abilities[Math.floor(Math.random()*abilities.length)] });
}
    ...
    for(const pid in players){
      ...
      if(Math.sqrt(dx*dx+dy*dy)<20){
        switch(s.type){
          ...
          case "dark": p.darkDebuff = Date.now() + 10000; break;
          ...
          case "wind": 
            p.vx = (p.x - s.x > 0 ? 1 : -1) * 5; 
            p.vy = (p.y - s.y > 0 ? 1 : -1) * 2; 
            p.windDebuff = Date.now() + 5000; 
            break;
        }
        spells.splice(i,1); 
        break;
...
// Apply debuffs and continuous effects
function applyDebuffs(){
  ...
  for(const id in players){
    ...
    if(p.hp <= 0){
      // Drop non-fire abilities
      const dropAbilities = p.abilities.filter(a => a !== "fire");
      dropAbilities.forEach(a => crates.push({...randomPos(), ability: a}));
      p.abilities = ["fire"];
      p.hp = 100;
      p.x = WORLD_SIZE/2; p.y = WORLD_SIZE/2;
    }
  }
}
